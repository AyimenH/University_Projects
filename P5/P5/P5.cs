...
public class P5
{
    static void Main()
    {
        ...
        Console.WriteLine("\n--- SubStream Testing ---");
        MsgStream[] heterogeneousSubArray = new MsgStream[4];
        
        for (int i = 0; i < heterogeneousSubArray.Length; i++)
        {
            heterogeneousSubArray[i] = getObj(i, file);
        }
        
        SubStream subStream = new SubStream(heterogeneousSubArray, 4);
        
        subStream.WriteMessage(0, string1);
        subStream.WriteMessage(1, string2);
        subStream.WriteMessage(2, string3);
        subStream.WriteMessage(3, string4);
        
        Console.WriteLine("Testing SubStream WriteMessage:");

        int[] subscribers1 = subStream.QuerySubscribers();
        
        for (int i = 0; i < subscribers1.Length; i++)
        {
            Console.WriteLine("Subscriber " + i + ": " + subscribers1[i]);
        }
        
        Console.WriteLine("\nTesting Multiple Writes to Same Partition:");
        
        subStream.WriteMessage(0, "Additional Message 1");
        subStream.WriteMessage(0, "Additional Message 2");

        int[] subscribers2 = subStream.QuerySubscribers();
        
        for (int i = 0; i < subscribers2.Length; i++)
        {
            Console.WriteLine("Subscriber " + i + ": " + subscribers2[i]);
        }
        
        Console.WriteLine("\n--- Other Stream Testing ---");
